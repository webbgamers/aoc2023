use std::{fmt::Display, iter::zip, ptr::write_unaligned};
...
fn part1(input: &str) -> impl Display {
	let patterns = input.split("\n\n").map(|pattern_str| {
		...
		let pattern_str = pattern_str.lines().flat_map(|l| l.chars()).collect::<Vec<_>>();
	...
	'outer: for (width, height, pattern) in patterns {
		...
		for mirror_x in 1..width {
			...
			for (left_x, right_x) in zip(((mirror_x - check_size)..(mirror_x)).rev(), (mirror_x)..(mirror_x + check_size)) {
				for y in 0..height {
		...
		for mirror_y in 1..height {
			...
			for (top_y, bottom_y) in zip(((mirror_y - check_size)..(mirror_y)).rev(), (mirror_y)..(mirror_y + check_size)) {
				for x in 0..width {
