...
fn part1(input: &str) -> impl Display {
    let mut hands = input
        ...
        .map(|l| {
            ...
            (l.next().unwrap(), l.next().unwrap().parse::<usize>().unwrap())
        })
        .collect::<Vec<_>>();

	let charmap = ['A', 'K', 'Q', 'J', 'T', '9', '8', '7', '6', '5', '4', '3', '2'];
...
fn part2(input: &str) -> impl Display {
	...
	let charmap = ['A', 'K', 'Q', 'T', '9', '8', '7', '6', '5', '4', '3', '2', 'J'];
...
fn find_type_p1(hand: &str) -> u8 {
	...
	for c in hand.chars() {
		chars.entry(c).and_modify(|counter| *counter += 1).or_insert(1);
	}
